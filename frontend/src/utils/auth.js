...
export const notifyAppNotice = (code, tone = "info") => {
  if (typeof window === "undefined") return;
  if (!code) return;
  const now = Date.now();
  if (lastNotice.code === code && now - lastNotice.at < NOTICE_THROTTLE_MS) {
    return;
  }
  lastNotice = { code, at: now };
  window.dispatchEvent(
    new CustomEvent(APP_NOTICE_EVENT, { detail: { code, tone } })
  );
};
